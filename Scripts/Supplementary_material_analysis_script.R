...
### We are interested in the disposition, demographic, and mathematical literacy scores
...
stdq <- fread("Data/Raw/PISA2012_StdQ_AUS.dat",
              select = c(identifiers_raw, demographics_raw, dispositions, disposition_indices, maths_literacy, stu_weights)
)
...
# Figure 5
p5 <- xgb.plot.importance(imp_model1, numberOfClusters = 1)
...
stdq2 <- stdq[, c(identifiers, demographics, disposition_indices, maths_literacy, stu_weights), with = F]
...
fit_model2 <-
  gbm.step(gbm.y = "PV1MATH",
           ...
           tree.complexity = 10,
           bag.fraction = 0.5,
           site.weights = stdq2[train_set_2, W_FSTUWT],
           verbose = T
  )

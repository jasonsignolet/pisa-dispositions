...
stdq <- fread("Data/Raw/PISA2012_StdQ_AUS.dat", 
               select = c(identifiers, demographics, dispositions, disposition_indices, maths_literacy)
)
...
for(col in dispositions) {
  set(stdq1, i = which(d[[col]] < 1), j = col, value = 1)
  set(stdq1, i = which(d[[col]] > 4), j = col, value = 4)
}
...
# Do the fitting
fit_model1 <- train(lrn_model1, task = task, subset = train_set)

# Make predictions from the test set
pred_model1 <- as.data.table(predict(fit_model1, task = task, subset = test_set))
...
error_table[, pred_model1[, quantile(abs(truth-response), c(.5, .9, .99))] ]
...
# Set controllers for hyperparameter tuning
task2 <- makeRegrTask(
  data = stdq2[train_set_2, c(demographics, disposition_indices, "PV1MATH"), with = F],
  target = "PV1MATH"
)
...
p8a <- gbm.perspec(fit_model2, 8, 6, z.range = c(350, 600), x.range = dr, y.range = dr)

# Figure 8b: ANXMAT 8 vs SCMAT 7
p8b <- gbm.perspec(fit_model2, 8, 7, z.range = c(400, 580), x.range = dr, y.range = dr)

# Figure 8c: ANXMAT 8 vs ESCS 2
p8c <- gbm.perspec(fit_model2, 8, 2, z.range = c(400, 580), x.range = dr, y.range = dr)

# Figure 8d: MATHEFF 6 vs ESCS 2
p8d <- gbm.perspec(fit_model2, 2, 6, z.range = c(300, 580), x.range = dr, y.range = dr)

# Figure 8e: ANXMAT 8 vs INTMAT 9
p8e <- gbm.perspec(fit_model2, 9, 8, z.range = c(400, 580), x.range = dr, y.range = dr)

# Figure 8f: ANXMAT vs GENDER
p8f <- gbm.perspec(fit_model2, 8, 1, z.range = c(425, 580), x.range = dr, y.range = c(0, 1))
